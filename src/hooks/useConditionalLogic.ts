...
export const useConditionalLogic = (
  ...
  /**
   * Enhanced save-triggered evaluation with multi-subcategory support
   */
  const evaluateOnSave = useCallback(async (
    ...
    try {
      setLoading(true);
      console.log('🔧 Starting enhanced multi-subcategory conditional logic evaluation...');
      ...
      // Use the enhanced evaluation method with multi-subcategory support
      ...
      // Smart conditional question management with proper multi-subcategory support
      if (newSubcategories.length > 0) {
        ...
        // Fetch enhanced additional questions with preserved answers (excludes initiators)
        ...
        if (error) {
          ...
          typedQuestions.forEach(q => {
            console.log('🔧 Additional question:', q.itemName, 'Type:', q.itemType, 'Subcategory:', q.subcategory);
          });
        }
      } else {
  ...
  /**
   * Load existing additional questions without triggering evaluation (excludes initiators)
   */
  const loadExistingAdditionalQuestions = useCallback(async (subcategories?: string[]) => {
    ...
    try {
      ...
      // Use the method that excludes initiator questions
      const { data, error } = await ConditionalLogicService.getAdditionalQuestionsBySubcategoriesWithPreservation(
        targetSubcategories,
        categoryId,
        projectId,
  ...
  const transformToTypedQuestions = useCallback((data: any[]): TypedChecklistItem[] => {
    const typedQuestions: TypedChecklistItem[] = data.map(item => {
      ...
      const subcategory = requiredItem?.subcategory || 
      ...
      return {
        ...
        subcategory, // Add subcategory for debugging/tracking
      };
    });
